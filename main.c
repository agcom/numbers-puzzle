...
void initGraphics(char *graphics[ROW_CHARS][COLUMN_CHARS], int nums[ROWS][COLUMNS]) {
	...
	for (i = 0; i < ROWS; i++) { //iterates over board rows
		...
		int j;
		for (j = 0; j < (i == 0 ? ROOM_HEIGTH + 2 : ROOM_HEIGTH +
		                                            1); j++) { //iterates over each room rows; has +1 row at first board's row
			
			int k;
			for (k = 0; k < COLUMNS; k++) { //iterates over board columns
				
				int l;
				for (l = 0; l < (k == 0 ? ROOM_WIDTH + 2 : ROOM_WIDTH +
				                                           1); l++) { //iterates over each room columns; has +1 column at first board's column
					
					//refer to ZERO_MASK, END_MASK
					int _i = (i == 0 ? ZERO_MASK : 0) | (i == ROWS - 1 ? END_MASK : 0);
					int _j = (j == 0 ? ZERO_MASK : 0) |
					         (j == (i == 0 ? ROOM_HEIGTH + 2 : ROOM_HEIGTH + 1) - 1 ? END_MASK : 0);
					...
					else if (l == (k == 0 ? 1 : 0) && j == (ROOM_HEIGTH / 2 + (i == 0 ? 1 : 0)))
